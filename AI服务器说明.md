...
AI 编曲服务器源码包含在 [本仓库](fluentmelody/servers/ai.py)，采用 [MIT 许可](LICENSE)，任何人都可以自行搭建。它与联机服务器独立运行，使用独立数据库、兑换 token 和余额。自建者可免费发放额度，也可自行定价；兑换码由自建者使用本地管理员工具生成，无需向作者申请。AI 服务的次数不会扣联机时长，提供方账户及调用费用由自建者负责。
