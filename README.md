...
Windows 口风琴演奏客户端，界面使用自研的 FluentPy。默认显示免费的单机演奏；多人合奏和 AI 编曲可在设置中开启 Beta 模式后使用。价格由运营者决定，程序不接入支付平台。
...
项目源码采用 [MIT 许可](LICENSE)，附带的自研 FluentPy 同样采用 [MIT 许可](licenses/FluentPy-LICENSE.txt)。第三方图标、模型与运行依赖保留各自的许可，详见 [第三方声明](licenses/THIRD_PARTY_NOTICES.md)。
...
发布包中的 `FluentMelodyServer.exe` 可分别启动两种独立进程，也可在服务器本机管理 token：

```powershell
.\FluentMelodyServer.exe multiplayer --db data/multiplayer.sqlite3 --host 127.0.0.1 --port 8765
.\FluentMelodyServer.exe ai --config ai.example.json
```

也可双击「启动联机服务.bat」或「启动AI服务.bat」。默认地址仅供本机使用，在客户端开启 Beta 模式后，于设置中分别填写 `http://127.0.0.1:8765` 和 `http://127.0.0.1:8766`。服务器运行期间保持其窗口打开。

AI 默认未配置，能够启动但不会接单扣费。需要先修改 `ai.example.json`，接入运营者自己的 API 或独立的本机 Codex 服务账户，详见 **AI服务器说明.md**。客户端不接触 API 密钥。
...
```powershell
# 生成一个一天的联机 token
.\FluentMelodyServer.exe admin --db data/multiplayer.sqlite3 --kind multiplayer issue --days 1 --count 1
# 生成 10 个一天的联机 token，并保存到新文件
.\FluentMelodyServer.exe admin --db data/multiplayer.sqlite3 --kind multiplayer issue --days 1 --count 10 --output tokens-001.txt
# 生成一个含 20 份额度的 AI token
.\FluentMelodyServer.exe admin --db data/ai.sqlite3 --kind ai issue --credits 20 --count 1
```

明文 token 只在生成时输出，请保管好。用户开启 Beta 模式后，在「多人合奏」或「AI 编曲」对应页面兑换。

联机 token 按你的规则，可按 **5 元/天/人**出售。每人购买自己的 token，首次兑换立即开始按自然时间倒计时，离线和暂停同样计时。首次兑换绑定本机设备密钥；原设备重新兑换同一码只恢复登录，不重复增加时长，另一设备不能兑换。新 token 的时长接在已有余额后；余额耗尽时从本次兑换时刻开始。
